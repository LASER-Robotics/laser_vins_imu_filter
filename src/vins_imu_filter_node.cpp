...
void VinsImuFilter::getParameters() {
  ...
  RCLCPP_INFO(get_logger(), "Accelerometer IIR Filter a: [%s]",
              std::accumulate(_acc_iir_a_.begin(), _acc_iir_a_.end(), std::string(), [](const std::string &a, double b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());

  get_parameter("accelerometer.iir_filter.b", _acc_iir_b_);
  RCLCPP_INFO(get_logger(), "Accelerometer IIR Filter b: [%s]",
              std::accumulate(_acc_iir_b_.begin(), _acc_iir_b_.end(), std::string(), [](const std::string &a, double b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());
  ...
  RCLCPP_INFO(get_logger(), "Accelerometer Notch Filter Frequencies: [%s]",
              std::accumulate(_acc_notch_frequencies_.begin(), _acc_notch_frequencies_.end(), std::string(), [](const std::string &a, int b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());
  ...
  RCLCPP_INFO(get_logger(), "Gyro IIR Filter a: [%s]",
              std::accumulate(_gyro_iir_a_.begin(), _gyro_iir_a_.end(), std::string(), [](const std::string &a, double b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());

  get_parameter("gyro.iir_filter.b", _gyro_iir_b_);
  RCLCPP_INFO(get_logger(), "Gyro IIR Filter b: [%s]",
              std::accumulate(_gyro_iir_b_.begin(), _gyro_iir_b_.end(), std::string(), [](const std::string &a, double b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());
  ...
  RCLCPP_INFO(get_logger(), "Gyro Notch Filter Frequencies: [%s]",
              std::accumulate(_gyro_notch_frequencies_.begin(), _gyro_notch_frequencies_.end(), std::string(), [](const std::string &a, int b) {
                return a.empty() ? std::to_string(b) : a + ", " + std::to_string(b);
              }).c_str());
...
void VinsImuFilter::initializeFilter() {
  ...
  imu_filter_ = std::make_unique<laser_uav_lib::ImuFilter>(_acc_iir_enable_, _acc_iir_a_, _acc_iir_b_, _acc_notch_enable_, _acc_notch_sample_rate_,
                                                           acc_notch_freq_matrix, _acc_notch_bandwidth_, _gyro_iir_enable_, _gyro_iir_a_, _gyro_iir_b_,
...
void VinsImuFilter::subGyroCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
...

  {
    ...
    filtered_msg.linear_acceleration_covariance = last_accel_msg_.linear_acceleration_covariance;
  }
...
laser_uav_lib::ImuData VinsImuFilter::convertToImuData(const sensor_msgs::msg::Imu &imu_msg) {
  ...
  imu_data.orientation[1] = imu_msg.orientation.y;
  imu_data.orientation[2] = imu_msg.orientation.z;
  imu_data.orientation[3] = imu_msg.orientation.w;

  std::copy(imu_msg.orientation_covariance.begin(), imu_msg.orientation_covariance.end(), imu_data.orientation_covariance.begin());
...
sensor_msgs::msg::Imu VinsImuFilter::convertToSensorMsg(const laser_uav_lib::ImuData &imu_data) {
  ...
  std::copy(imu_data.orientation_covariance.begin(), imu_data.orientation_covariance.end(), imu_msg.orientation_covariance.begin());
  ...
  std::copy(imu_data.angular_velocity_covariance.begin(), imu_data.angular_velocity_covariance.end(), imu_msg.angular_velocity_covariance.begin());
  ...
  std::copy(imu_data.linear_acceleration_covariance.begin(), imu_data.linear_acceleration_covariance.end(), imu_msg.linear_acceleration_covariance.begin());
